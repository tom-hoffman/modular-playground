...
class PlayerModel(object):
    def __init__(self, bank_button_count=0, bank_cc_count=0, 
        ...
        self.sample_index = update_index(sample_button_count, sample_cc_count, config.SAMPLE_COUNT)
        self.cc_keys = list(config.CC_VALUES)
        self.wav = wav
        self.update_display = False

    def update_bank_index(self):
          update_index(self.bank_button_count, self.bank_cc_count, len(config.BANKS))
          self.update_display = True

    def update_sample_index(self):
          update_index(self.sample_button_count, self.sample_cc_count, config.SAMPLE_COUNT)
          self.update_display = True
